...
export const TrainingPage: React.FC = () => {
  ...
  const generateAIResponse = async (userMessage: string, currentStep: any, allSteps: Step[]) => {
    // Send a compact steps array so the body stays small
    const compact = allSteps.map((s, i) => ({
      stepNumber: i + 1,
      title: s.title,
    ...
    try {
      const data = await api(API_ENDPOINTS.AI_CONTEXTUAL_RESPONSE, {
        ...
        body: JSON.stringify({
          userMessage,
          currentStep,
          allSteps: compact,   // keep name but compact the content
          videoTime,
          moduleId
        }),
      })
      ...
      return text || generateFallbackResponse(userMessage, currentStep, allSteps)
    } catch (error) {
  ...
  const generateFallbackResponse = (userMessage: string, currentStep: any, allSteps: Step[]) => {
    ...
    const wordKey = Object.keys(ordinals).find(w => msg.includes(`${w} step`) || msg.includes(`the ${w} step`))
    ...
    if (msg.includes('time') || msg.includes('duration') || msg.includes('how long')) {
      if (currentStep) {
        ...
        return `Step ${currentStep.stepNumber} lasts ~${dur}s.`
      }
